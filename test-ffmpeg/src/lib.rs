/// FFMPEG-based testing code.
///
/// This is a separate crate to workaround cargo's inability to specify optional dev-dependencies (see
/// rust-lang/cargo#1596).

#[cfg(not(feature = "ffmpeg"))]
/// Read `data` using ffmpeg, verifying that the demuxed frames match the `expected_mdat_data`.
pub fn verify_ffmpeg(_data: &[u8], _expected_mdat_data: &[u8]) {
    log::info!("not verifying sanitizer output using ffmpeg; ffmpeg-tests feature disabled");
}

#[cfg(feature = "ffmpeg")]
pub use ffmpeg::verify_ffmpeg;
